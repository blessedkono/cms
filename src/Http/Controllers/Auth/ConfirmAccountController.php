...
class ConfirmAccountController extends Controller
{
    ...
    public function confirm($token)
    {
        ...
        return redirect()->route('cms.dashboard.index')->withFlashSuccess(trans('exceptions.auth.confirmation.success'));
    }
    ...
    public  function smsConfirm(Request $request,User $user)
    {
        ...
        if(strcmp($code,$token) == 0){
            $this->user->confirmAccount($token);
            return redirect()->route('cms.dashboard.index')->withFlashSuccess(__('exceptions.auth.confirmation.success'));
        }
        else{
